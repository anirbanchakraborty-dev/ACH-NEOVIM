...
return {
  ...
  -- adoption happens.
  {
    ...
    config = function(_, opts)
      ...
      -- run.sh template provider (SystemVerilog / Verilog projects).
      --
      -- Detects ./run.sh in cwd or above and exposes every subcommand
      -- as a parameterized task. Pairs with the direct <leader>R*
      -- keymaps in lang.lua: those cover the high-frequency loop
      -- (lint/fmt/sim/wave/synth/schematic), this template covers the
      -- long tail (lint:errors / lint:style / fmt:check / sim:unit /
      -- sim:integration) and gives you task list + history.
      --
      -- Implemented as a `generator`-style provider (not a static
      -- template) because the visibility gate is dynamic per project:
      -- the generator walks up from the search dir to find run.sh and
      -- yields the template only when found, with the discovered
      -- script directory baked into the closure. This is the right
      -- primitive for "show this template only when ./run.sh exists
      -- in the project tree" -- the static template's `condition`
      -- table only supports filetype/dir/tags filters, not arbitrary
      -- callbacks.
      --
      -- The provider also carries `condition.filetype = { systemverilog,
      -- verilog }` so non-SV buffers don't even invoke the generator.
      -- Single template with an enum subcommand param so <leader>oo
      -- doesn't drown in 14 entries.
      local sv_filetypes = { "systemverilog", "verilog" }
      overseer.register_template({
        name = "run.sh",
        condition = { filetype = sv_filetypes },
        generator = function(search, cb)
          local found = vim.fs.find("run.sh", { upward = true, path = search.dir, type = "file" })[1]
          if not found then
            cb({})
            return
          end
          local script_dir = vim.fs.dirname(found)
          cb({
            {
              name = "run.sh",
              desc = "Run ./run.sh <subcommand>",
              builder = function(params)
                local cmd = { "./run.sh", params.subcommand }
              ...
              params = {
                ...
                target = {
                  type = "string",
                  desc = "Optional target (testbench / module name) for sim / wave / synth / schematic",
                  optional = true,
                  order = 2,
                },
              },
